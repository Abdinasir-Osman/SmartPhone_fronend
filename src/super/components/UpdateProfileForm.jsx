...
export default function UpdateProfileForm({ profile }) {
  ...
  const handleEditClick = () => {
    if (editing) {
      document.getElementById('superadmin-profile-image-upload').click();
    } else {
      setEditing(true);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex flex-col items-center space-y-4 mb-6">
          <div className="relative">
            ...
            <input
              id="superadmin-profile-image-upload"
              type="file"
              className="hidden"
        ...
        {editing ? (
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow w-full max-w-xs mx-auto sm:mx-auto my-6 sm:my-8 text-center space-y-3 px-4">
             ...
             <p className={`text-sm font-semibold ${profile.status === "active" ? "text-green-600 dark:text-green-400" : "text-red-500 dark:text-red-400"}`}>
               {profile.status}
             </p>
          </div>
        )}
